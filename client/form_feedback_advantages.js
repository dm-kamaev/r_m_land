...
// ФОРМА ОБРАТНОЙ СВЯЗИ ПРИ КЛИКЕ НА ОДИН ИЗ ПУНКТОВ ADVANTAGES
// TODO: Грязный код

var form_feedback_advantages = (function () {
  "use strict";
  var exports = {};
  ...
  var TREE = {
    ...
    after_submit: { id: main_2, style: 'margin-top:30px', class: '"container center-block"'},
  ...
  function go_to_form () {
    getByID(TREE.after_submit_goToForm.id).onclick = function (e) {
      ...
      // getByID('form_feedback_service').style.display = 'block';
      getByID(TREE.after_submit.id).innerHTML = '';
    };
  }

  function html_advantages () {
    var html = '';
    ...
    html += '<div id='+TREE.formFeedback_orderCall.id+' style="float:none;margin:0 auto;" class="col-md-8 contact-right">';
      html += '<form action=#>';
    ...
    html += '<div id='+TREE.formFeedback_sendEmail.id+' style="float:none;margin:0 auto;margin-top:20px;" class="col-md-8 contact-right">';
      html += '<form action=#>';
  ...
  var proccesing_services_content = function(id) {
    ...
    if (m && m[1]) {
      ...
      getByID(TREE.formFeedback_sendEmail_message.id).value = subject+':';
      ...
      fadeInOut(getByID('services'),'Out','none', 50, function() {
        ...
        fadeInOut(getByID(TREE.formFeedback.id), 'In', 'block', 50);
        scrollTo(TREE.formFeedback.id, 1000);
      });
    }
  };
  ...
  exports.render = function() {
    var html = '';
    // html += '<div id=form_feedback_service style=opacity:1 class=contact-us>';
    html += '<div id='+TREE.formFeedback.id+' style=opacity:1 class=contact-us>';
      html += '<div class=container>'+html_advantages()+'</div>';
    html += '</div>';
    html += '<div id='+TREE.after_submit.id+' style='+TREE.after_submit.style+' class='+TREE.after_submit.class+'></div>';
    getByID('services').insertAdjacentHTML('beforeBegin', html);
    // getByID('form_feedback_service').style.display = 'none';
    getByID(TREE.formFeedback.id).style.display = 'none';
    getByID('cross').onclick = function() {
      fadeInOut(getByID(TREE.formFeedback.id),'Out','none', 50, function() {
  ...
  function html_show_right_answer (type_order, text_connection) {
    return '<div style=text-align:center;font-size:170%;font-family:\'Lato\', sans-serif;>'+
